...
public class FieldPhotoProfileDetector implements DcpProfileDetector {

    private final static String DRYVALLEYSROOT = "McMurdoDryValleys";
    private final static String FIELDPHOTOSELEMENT = "FieldPhotos";
    private final static String PHOTOGRAPHERLATITIUDE = "photographerLatitude";
    
    @Override
    public boolean hasFormat(Dcp dcp) {
        boolean isFieldPhoto = false;
           ...
           for( DcsDeliverableUnit du : dcp.getDeliverableUnits()){
               if( du.getMetadata().iterator().hasNext()){
                   ...
                   if( parser.getRootNodeName().equalsIgnoreCase(DRYVALLEYSROOT) && 
                           ...
                           parser.hasElement(PHOTOGRAPHERLATITIUDE)) {
                       isFieldPhoto = true;
                   }
               }
               if( !isFieldPhoto && du.getMetadataRef().iterator().hasNext()){
