...
public class DcpIndexService implements IndexService<Dcp> {
    private final SolrService solr;
    private final ArchiveStore store; // needed to retrieve DcsFile datastreams

    public DcpIndexService(ArchiveStore store) throws IndexServiceException {
        try {
    ...
    public DcpIndexService(ArchiveStore store, SolrService solr) {
        this.store = store;
        this.solr = solr;
    }

    public DcpIndexService(SolrService solr) {
        this.store = null;
        this.solr = solr;
    }

    public BatchIndexer<Dcp> index() throws IndexServiceException {
        return new DcpBatchIndexer(solr, store);
    }
