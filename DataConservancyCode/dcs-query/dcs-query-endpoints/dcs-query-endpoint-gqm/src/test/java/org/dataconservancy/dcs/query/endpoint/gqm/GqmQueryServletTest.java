...
public class GqmQueryServletTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        two.getLocations().add(
                new Location(new Geometry(Geometry.Type.LINE, new Point(3, 0),
                        new Point(8, 0)), srid));
    ...
    public void testGET_Gqm() throws Exception {
        ...
        request.setURI("/qf/query/"
                + ServletUtil
                        .encodeURLPath("intersects([line 'EPSG:4326' 4 -1, 4 1])"));
        ...
        Assert.assertEquals("" + gqms.getGQMs().size(),
                response.getHeader("X-TOTAL-MATCHES"));

        GQMList g = gqmBuilder.buildGQMList(new ByteArrayInputStream(response
                .getContent().getBytes()));
        ...
        request.setURI("/qf/query/"
                + ServletUtil
                        .encodeURLPath("intersects([line 'EPSG:4326' 4 1, 4 4])"));
        ...
        // Invalid test
        request.setURI("/qf/query/" + ServletUtil.encodeURLPath("garbageQuery"));
        response.parse(servletContainer.getResponses(request.generate()));
        Assert.assertEquals(404, response.getStatus());

        // Invalid test
        request.setURI("/qf/gqm/" + ServletUtil.encodeURLPath("Location:560"));
        response.parse(servletContainer.getResponses(request.generate()));
        Assert.assertEquals(404, response.getStatus());
    }
    ...
    public void testHEAD() throws Exception {
        ...
        Assert.assertEquals("" + gqms.getGQMs().size(),
                head.getResponseHeader("X-TOTAL-MATCHES").getValue());
