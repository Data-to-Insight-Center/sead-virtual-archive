 ...
 *
 */
public abstract class MockFileBusinessObjectSearcher {
    final MockArchiveUtil archiveUtil;

    public MockFileBusinessObjectSearcher(MockArchiveUtil archiveUtil) {
    ...
    public BusinessObjectState findLatestState(String business_id) {
        ...
        next: for (DcsDeliverableUnit du : dus) {
            for (DcsDeliverableUnit du2 : dus) {
                ...
                if (pred_id == null && !du.getParents().isEmpty() && du.getType().equals(DataFileProfile.DATAFILE_ROOTDU_TYPE)) {
                    rootDu = du;
                    continue next;
                }
    ...
    public DcsFile findFile(String business_id, String type) {
        for (Set<DcsEntity> entities : archiveUtil.getEntities().values()) {
            for (DcsEntity entity : entities) {
