...
public class DUMapper
    ...
    public FedoraDigitalObject map(DcsEntity in) throws AIPFormatException {
        ...
        DcsDeliverableUnit entity = null;
        if ((in != null) && (DcsDeliverableUnit.class == in.getClass())) {
            entity = (DcsDeliverableUnit) in;
        } else {
        ...
        // Default Title
        String title = "Deliverable Unit: " + entity.getId();
        if (entity.getTitle() != null) {
            title = entity.getTitle();
        }

        // Object Properties for a DU
        ...
        erdfecmodel.setResource("info:fedora/dcs:DeliverableUnit");
        ...
        Datastream duds = new Datastream();
        duds.setId(DatastreamConverter.DCPXML);
        duds.setControlGroup(DatastreamConverter.CONTROL_GROUP_X);
        duds.setState(DatastreamConverter.STATE_A);
        duds.setVersionable(DatastreamConverter.VERSIONABLE_T);

        DatastreamVersion dudsv = new DatastreamVersion();
        dudsv.setVersionID(duds.getId()
                + DatastreamVersionConverter.BASE_VERSION);
        dudsv.setLabel(DatastreamVersionConverter.DEFAULT_LABEL);
        dudsv.setFormatURI(DatastreamVersionConverter.DEFAULT_FORMAT_URI);
        dudsv.setMimeType(DatastreamVersionConverter.XMLCONTENT_MIME);

        XMLContent duxmlc = new XMLContent();

        dcp.getContentList().add(entity);
        duxmlc.setContent(dcp);
        dudsv.setContent(duxmlc);
        duds.getVersionList().add(dudsv);
        fdo.getDatastreamMap().put(duds.getId(), duds);
