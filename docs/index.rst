...
- Creating an ACR account: (Not needed for local bag upload) Please request an account in `Active Content Repository`
.. http://sead-demo.ncsa.illinois.edu/acr/
- Build requirements: For building code please use maven and jdk (1.7.x) Please setup latest version of Apache Tomcat Once tomcat is setup, please copy `Hibernate` jar and `mysql-connector` jar into the into the lib folder inside tomcat folder.
