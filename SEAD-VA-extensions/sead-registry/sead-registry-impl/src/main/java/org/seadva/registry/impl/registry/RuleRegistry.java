...
import org.seadva.registry.impl.resource.RuleResource;

import java.util.List;

/**
 * Read and write rule resource to registry
 */
public class RuleRegistry implements Registry {

    BaseDaoImpl baseDao;
    RuleRegistry(BaseDaoImpl baseDao){
    ...
    public boolean putResource(ResourceType resourceType, Resource resource) {
        try {
            EntityDao entityDao = ((RuleResource)resource).getEntityDao();
            boolean insertEntity = this.baseDao.entityDaoImpl.insertEntity(entityDao);
            boolean insertProperties = true;
            for(PropertyDao propertyDao:((RuleResource)resource).getPropertyDao()){
            ...
            for(EntityTypeDao entityTypeDao:((RuleResource)resource).getEntityTypeDao()){
