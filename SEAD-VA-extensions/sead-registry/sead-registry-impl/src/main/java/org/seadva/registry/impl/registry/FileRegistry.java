...
import org.seadva.registry.impl.resource.FileResource;

import java.util.List;

/**
 * Read and write file resource to registry
 */
public class FileRegistry implements Registry {

    BaseDaoImpl baseDao;
    FileRegistry(BaseDaoImpl baseDao){
    ...
    public Resource getResource(String entityId) {
        try {
            ...
            return new FileResource(entityDao, entityTypeDao, propertyDao);
        } catch (Exception e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
        }
        return new FileResource();  //To change body of implemented methods use File | Settings | File Templates.
    }

    @Override
    public boolean putResource(ResourceType resourceType, Resource resource) {
        try {
            EntityDao entityDao = ((FileResource)resource).getEntityDao();
            boolean insertEntity = this.baseDao.entityDaoImpl.insertEntity(entityDao);
            boolean insertProperties = true;
            for(PropertyDao propertyDao:((FileResource)resource).getPropertyDao()){
            ...
            for(EntityTypeDao entityTypeDao:((FileResource)resource).getEntityTypeDao()){
