...
import org.dataconservancy.dcs.access.client.model.DcpTreeModel2;
...
public class PublishDataPresenter implements Presenter {
	...
	private void registerPreviewUpdate(){
		final EntityEditEvent.Handler editHandler = new EntityEditEvent.Handler() {
			...
			//	outerPanel.add(Util.label("Your SIP is ready for review by your Institutional Repository.", "TopHeaderText"));
				...
				TreeViewModel treemodel = new DcpTreeModel2(editEvent.getDcp());
				...
				for(int i =0; i<dus.length();i++){
					...
					{
						projectNameTB.setText(dus.get(i).getCoreMd().getTitle());
						abstractTB.setText(dus.get(i).getAbstract());
							...
							for(int j =0; j<alternateIds.length();j++){
								...
								if(alternateId.getTypeId().contains("doi")){
									...
									warningLabel.setText("It seems like this dataset was already published and " +
											"has an identifier." +
											"Would you Virtual Archive to track provenance to this Published dataset?");
							...
							SeadApp.userService.checkSession(null, new AsyncCallback<UserSession>() {
			        			...
			        			public void onSuccess(final UserSession userSession) {
									...
									if(provenanceType!=null){
										registryService.updateSip(editEvent.getSipPath(), roId.getText(), provenanceType.getText(), roId.getText(), new AsyncCallback<Void>() {
											
											@Override
											public void onSuccess(Void result) {
												startWorkflow(editEvent.getSipPath());
											}
											@Override
											public void onFailure(Throwable caught) {
	...
	void startWorkflow(final String sipPath){
		 mediciService.generateWfInstanceId(new AsyncCallback<String>() {
				
			 @Override
				public void onSuccess(final String wfInstanceId) {
				 MessagePopupPanel popupPanel = new MessagePopupPanel("Starting workflow. Please view status in the monitor panel.","wait", true);
				 popupPanel.show();
				 mediciService.submitMultipleSips(
						...
						wfInstanceId,
						null,
						0, 0, "", "", false, GWT.getModuleBaseURL(), SeadApp.tmpHome,true,
						new AsyncCallback<String>() {
							
							@Override
							public void onSuccess(String statusUrl) {
//									Window.alert("Done:" + result);
								MessagePopupPanel popUpPanel = new MessagePopupPanel(statusUrl, "done", true);
								popUpPanel.show();
