...
public class Search {
    ...
    public enum UserField {
                ...
                TITLE(
                		...
                		SolrField.FileField.NAME
                ),
                ABSTRACT(
                		"Abstract", SolrField.CoreMetadataField.ABSTRACT
                ),
                CREATOR(
                		"Data Contributor", SolrField.CoreMetadataField.CREATOR
                ),
                
                DATE(
                		"Date", SolrField.CoreMetadataField.DATE
                ),
                LOCATION(
                		"Location", SolrField.CoreMetadataField.LOCATION
                ),
                METADATA(               
                		"Metadata Standard", DeliverableUnitField.METADATA_REF,
                		SolrField.MetadataField.SCHEMA,
                		SolrField.MetadataField.SEARCH_TEXT,
                		SolrField.MetadataField.TEXT
                		
                ),
        		ID("Id", SolrField.EntityField.ID,
                        SolrField.DeliverableUnitField.FORMER_REFS), 
                ALL("All",SolrField.EntityField.ID,
                        SolrField.DeliverableUnitField.FORMER_REFS,
                        SolrField.CoreMetadataField.TITLE,
                		SolrField.FileField.NAME,
                		SolrField.CoreMetadataField.ABSTRACT,
                		SolrField.CoreMetadataField.CREATOR,
                		SolrField.CoreMetadataField.DATE,
                		SolrField.CoreMetadataField.LOCATION,
                		DeliverableUnitField.METADATA_REF,
                		SolrField.MetadataField.SCHEMA,
                		SolrField.MetadataField.SEARCH_TEXT,
                		SolrField.MetadataField.TEXT
                        ),
             /*   Fgdc_Title(
                		"Fgdc-Title", SolrField.FgdcField.TITLE
                ),
                Fgdc_Abstract(
                		"Fgdc-Abstract", SolrField.FgdcField.ABSTRACT
                ),*/
                
          /*      FILE("File",
                SolrField.FileField.SOURCE,
    ...
    public static String createQuery(UserField[] userfields,
        ...
        if(userfields.length==0 && userqueries.length==0)
        	sb.append("*:*");

        String temp  =sb.toString();
        for(int i=0;i<facetField.length;i++)
        {
        	String field = facetField[i];
        	//if(facetField[i].equalsIgnoreCase("author"))
        		//f = "creator";
        	Constants constants = new Constants();
        	String facetValueStr = //facetValue[i].substring(0,facetValue[i].lastIndexOf('(')).replace(":", "\\:");
        			facetValue[i].replace(":", "\\:");
        	String tempStr =constants.facetValues.get(facetValueStr);
        	if(tempStr!=null)
        		facetValueStr = tempStr;
        	temp+="("+temp+") AND "+field+":"+"((\""+facetValueStr+"\") )";
        }
        return temp;
    }
    ...
    public static String createGqmQuery(StringBuilder sb,
            String field,
            String query) {
				if (field != null) {
					sb.append(field);
