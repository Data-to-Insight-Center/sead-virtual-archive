...
public class MediciServiceImpl extends RemoteServiceServlet implements
    ...
    public HashMap<String,HashMap<String,String>> submitSIPFile(String endpoint,
                                                                ...
                                                                boolean curation,
                                                                boolean repub) throws RPCException {
        ...
        try {
            ...
            for(DcsDeliverableUnit du:dus){
                if(du.getParents()==null||du.getParents().size()==0){
                    du.setAlternateIds(new HashSet<DcsResourceIdentifier>());
                    if(restrictAccess)
                        du.setRights("restricted");
                    if(curation)
                        du.setType("CurationObject");
                    else if(repub)
                        du.setType("PublishedObject-reattempt");
                    else
                    ...
                    if(curation){
                        String vivoId = (String)getThreadLocalRequest().getSession().getAttribute("vivoId");
                        if(vivoId!=null){
                            SeadPerson contributor = new SeadPerson();
                            contributor.setId(
                                    vivoId);
                            //submitter.setIdType("sead-email");
                            contributor.setIdType("vivo");
                            contributor.setName((String)getThreadLocalRequest().getSession().getAttribute("fName")
                                    +" "+(String)getThreadLocalRequest().getSession().getAttribute("lName"));
                            ((SeadDeliverableUnit)du).addDataContributor(contributor);
                        }
                    }
                }
                if(!curation){
...
//			    		   new FileInputStream(sipFilePath),
    ...
    public String submitMultipleSips(String sipEndpoint,
        ...
        if(datasetId==null){
            ...
            datasetId =// "tag:cet.ncsa.uiuc.edu,2008:/bean/Collection/"+
                    arr[arr.length-1];
        }
...
//													latestDate
    ...
    public String getBag(String tagId, MediciInstance sparqlEndpoint, String bagitEp, String tmpHome) {
        ...
        tmpHome  = System.getProperty("java.io.tmpdir")+"/";
        String splitChar = "/";
        if(!tagId.contains("/"))
            splitChar = ":";
