...
{ 
    ...
    @Override
    public boolean emailResearcher(String agentId, String entityUrl) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException{
        if(agentId==null)
            agentId = "";
        List<Person> persons = getPersonJdbc().getAllUsers("registryId", agentId, "string");
        List<String> toAddress = new ArrayList<String>();
        if(persons!=null)
            for(Person person:persons){
                toAddress.add(person.getEmailAddress());
            }

        String[] toAddressArr = toAddress.toArray(new String[toAddress.size()]);
        emailSender.sendEmail(toAddressArr, "Research Object accepted and approved", "Please note that your research object has been submitted to the" +
                " Institutional Repository and additional details can be viewed at "+ entityUrl +" .");
        return true;
    }
    @Override
    ...
    public void updateAllUsers(List<Person> userList, List<Person> sendEmailList, String registryUrl) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, IOException
    {
        ...
        for(Person person:sendEmailList)
            newUsers.add(person.getEmailAddress());

        for (Person user : userList) {
            if(newUsers.contains(user.getEmailAddress()))
    ...
    @Override
    public Person getUser(String registryId) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
        List<Person> persons =  getPersonJdbc().getAllUsers("registryid", registryId, "string");
